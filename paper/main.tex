...
In this paper we present the development of a self-sufficient cattle tracker prototype capable of monitoring a single cow for extended periods of time.
The tracker should record and collect data at regular intervals and transmit the data to the cloud with a maximum specified delay.
Solar power can be utilized to extend the power budget of the device.
Given that the device is capable of generating power, it should use excess power to transmit data more frequently, but the exact strategy is not trivial to determine.
Reinforcement learning is used to optimize the power consumption of the device by making decisions on when to transmit data, when to collect and store data, and when to do nothing.
