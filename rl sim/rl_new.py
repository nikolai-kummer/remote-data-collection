...
def transition(state, action, power_level, solar_intensity_cache):
    ...
    if action == 0:
        ...
        power_used = 5  # Assuming collecting data costs 5 power units
        message_count = min(MAX_MESSAGES, message_count + 1)
    elif action == 2:
        power_used = 10  # Base power usage for sending messages
        legitimate_messages_sent = message_count+1
        message_count = 0  # Clear the message queue

    # Update power level
    if power_level > power_used:
        power_level -= power_used
    else:
        print('Power level is 0 at time: ', time)
        power_level = 0
        legitimate_messages_sent = 0
    power_level = max(0, power_level)

    # Add generated solar power
    generated_power = solar_intensity_cache[int(time)]
    power_level += generated_power
    power_level = min(power_level, 100-1)

    # Increment time
    time = (time + 1) % N_TIME_INTERVALS

    return encode_state(power_level, time, message_count), power_level,  legitimate_messages_sent
...
if __name__ == '__main__':
    for episode in range(num_episodes):
        ...
        for t in range(int(N_TIME_INTERVALS*3)):  # Loop for each time interval
            ...
            # Perform the action
            next_state, current_power, legitimate_messages = transition(current_state, action, current_power, solar_intensity_cache)
